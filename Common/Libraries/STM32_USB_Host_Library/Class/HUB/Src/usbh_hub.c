...
static USBH_StatusTypeDef USBH_HUB_Process(USBH_HandleTypeDef * phost)
{
	...
	switch (HUB_Handle->state)
	{
		...
		case HUB_LOOP_PORT_CHANGED:
		  if(HUB_Handle->cur_port != last_port)
		  {
		    HUB_Handle->cur_port = HUB_GetPort(&(HUB_Handle->ports_change));
		    husb = (USBH_HandleTypeDef *) &(HUB_Handle->husb[HUB_Handle->cur_port - 1]);

		    if(husb->gState == HOST_IDLE || husb->gState == HOST_CLASS)
		    {
		      last_port = HUB_Handle->cur_port;
		      HUB_Handle->state = HUB_PORT_CHANGED;
		    }
		  }
		  else
		  {
		    HUB_Handle->ports_changed = 0;
		    HUB_Handle->state = HUB_IDLE;
		  }
		  break;
...
static void HUB_DeviceConnect(USBH_HandleTypeDef * phost, uint8_t port, uint8_t speed)
{
	...
	/* Evaluate topology: If the host/hub is in High-Speed ??but the child device is NOT */
    if ((phost->device.speed == USBH_SPEED_HIGH) && (husb->device.speed != USBH_SPEED_HIGH))
    {
        husb->device.DoSplit = 1;
        husb->device.HubAddr = phost->device.address; /* Address of the Hub that serves as a translator */
        husb->device.HubPort = port;                  /* Hub specific port */
    }
    else
    {
        husb->device.DoSplit = 0;
        husb->device.HubAddr = 0;
        husb->device.HubPort = 0;
    }	
...
USBH_HandleTypeDef * HUB_Process(USBH_HandleTypeDef * phost)
{
  ...
  if(status == USBH_OK)
  {
    ...
    {
      HUB_HandleTypeDef * HUB_Handle = (HUB_HandleTypeDef *) phost->pActiveClass->pData;
      HCD_HandleTypeDef * ph = (HCD_HandleTypeDef *) phost->pData;

      if (ph->hc[phost->Control.pipe_in].split_running == 0U && \
