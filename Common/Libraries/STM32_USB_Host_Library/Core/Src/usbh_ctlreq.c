...
static USBH_StatusTypeDef USBH_HandleControl(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->Control.state)
  {
    ...
    case CTRL_SETUP_WAIT:
      ...
      {
        phost->Control.timer_delay = HAL_GetTick(); 
        phost->Control.state = CTRL_SETUP_DELAY;
    ...
    case CTRL_SETUP_DELAY:
      if ((HAL_GetTick() - phost->Control.timer_delay) >= 2U)
      {
        phost->Control.state = CTRL_SETUP; 
      }
      break;      
    ...
    case CTRL_DATA_IN_WAIT:
      ...
      {
        if (phost->device.DoSplit == 1U)
        {
          phost->Control.timer_delay = HAL_GetTick(); 
          phost->Control.state = CTRL_DATA_IN_DELAY;
        }
    ...
    case CTRL_DATA_OUT_WAIT:
      ...
      else if (URB_Status == USBH_URB_NOTREADY)
      {
        ...
        phost->Control.timer_delay = HAL_GetTick(); 
        phost->Control.state = CTRL_DATA_OUT_DELAY;
    ...
    case CTRL_STATUS_IN_WAIT:
      ...
      else if (URB_Status == USBH_URB_NOTREADY)
      {
        ...
        {
          phost->Control.timer_delay = HAL_GetTick(); 
          phost->Control.state = CTRL_STATUS_IN_DELAY;
        }
    ...
    case CTRL_STATUS_OUT_WAIT:
      ...
      {
        phost->Control.timer_delay = HAL_GetTick(); 
        phost->Control.state = CTRL_STATUS_OUT_DELAY;
