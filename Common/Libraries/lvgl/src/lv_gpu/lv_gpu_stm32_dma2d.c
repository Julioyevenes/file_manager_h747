 ...
 */
void lv_gpu_stm32_dma2d_copy(lv_color_t * buf, lv_coord_t buf_w,
                             const lv_color_t * map_org, const lv_color_t * map, lv_coord_t map_w,
                             lv_coord_t copy_w, lv_coord_t copy_h,
                             lv_opa_t cf, lv_opa_t css)
{
    uint32_t offset = 0;
    
    if(cf == LV_IMG_CF_USER_ENCODED_0 && map != map_org) return;
    ...
    DMA2D->FGPFCCR = (uint32_t) ((cf == LV_IMG_CF_USER_ENCODED_0) ? LV_DMA2D_YCbCr : LV_DMA2D_COLOR_FORMAT)
                     | (css << DMA2D_FGPFCCR_CSS_Pos);
    DMA2D->FGMAR = (uint32_t)map;
    DMA2D->FGOR = map_w - copy_w + offset;
