                                         ...
                                         lv_opa_t cf, lv_opa_t css)
{
    ...
    if(disp->driver.set_px_cb) {
        ...
        map_normal(disp_area, disp_buf, &draw_area, map_area, map_buf, opa, mask, mask_res, cf, css);
    }
#if LV_USE_BLEND_MODES
    else {
...
#if LV_USE_GPU_STM32_DMA2D
            if(lv_area_get_size(draw_area) >= 240) {
                lv_gpu_stm32_dma2d_copy(disp_buf_first, disp_w, map_buf, map_buf_first, map_w, draw_area_w, draw_area_h, cf, css);
                return;
            }
#endif
