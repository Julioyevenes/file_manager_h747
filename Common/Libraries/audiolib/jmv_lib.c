...
/* Private constants ---------------------------------------------------------*/
#define JMV_LIB_READBUFF_SIZE   (8 * 1024 * 1024)
#define JMV_FRAME_TIMEOUT    	  100U
...
static jmv_lib_codec_t jmv_codec;
static jmv_lib_state_t jmv_lib_state;

/* Private function prototypes -----------------------------------------------*/
void jmv_lib_process(audio_lib_handle_t * hlib);
void jmv_lib_seek(audio_lib_handle_t * hlib);
...
void jmv_lib_process(audio_lib_handle_t * hlib)
{
    static jmv_lib_decode_state_t decode_state;
    ...
    switch (jmv_lib_state)
    {
        case JMV_LIB_READ_HEADER:
            ...
            hlib->local.buffer.size = JMV_LIB_READBUFF_SIZE;
        ...
        case JMV_LIB_READ_FRAMEVECT:
            ...
            f_read (hlib->fp, jmv_codec.metadata.frame.vect_addr, jmv_codec.metadata.frame.vect_size, (UINT *) &bytesread);
            ...
            jmv_lib_state = JMV_LIB_FILL_READBUFF;
            read_state = AUDIO_LIB_READ_REQ;
            break;
            
        case JMV_LIB_FILL_READBUFF:
            switch (read_state)
            {
        ...
        case JMV_LIB_FIRST_FRAME:
            switch (decode_state)
            {
                case JMV_LIB_DECODE_INIT:
                    ...
                    jmv_codec.expected_play_cur = hlib->local.play_cur;
                ...
                case JMV_LIB_DECODE_WAIT:
                    ...
                    else if ((HAL_GetTick() - decode_start_tick) > JMV_FRAME_TIMEOUT)
                    {
...
static void JMV_DecodeFrame(audio_lib_handle_t * hlib)
{
    ...
    if (hlib->local.play_cur != codec->expected_play_cur)
    {
        if (codec->expected_play_cur > hlib->local.play_cur) {
            diff = codec->expected_play_cur - hlib->local.play_cur;
            hlib->local.buffer.rptr = (hlib->local.buffer.rptr + diff) % hlib->local.buffer.size;
        } else {
            diff = hlib->local.play_cur - codec->expected_play_cur;
            hlib->local.buffer.rptr = (hlib->local.buffer.size + hlib->local.buffer.rptr - diff) % hlib->local.buffer.size;
        }
        hlib->local.play_cur = codec->expected_play_cur;
    }    
    ...
    while (codec->video_pos < codec->frame_pos) {
        ...
        if ((hlib->local.play_cur + skip_size) >= hlib->fp->fptr)
        {
            hlib->local.buffer.empty = 1;
            return;
        }

        hlib->local.buffer.rptr = (hlib->local.buffer.rptr + skip_size) % hlib->local.buffer.size;
        hlib->local.play_cur += skip_size;
    ...
    if ((hlib->local.play_cur + audio_pkt_size + jpeg_size) >= hlib->fp->fptr) 
    {
        hlib->local.buffer.empty = 1;
        return;
    }
    
    hlib->local.buffer.rptr = (hlib->local.buffer.rptr + audio_pkt_size) % hlib->local.buffer.size;
    hlib->local.play_cur += audio_pkt_size;
    
    /* Save pointers expected state after jpg decoding */
    codec->expected_play_cur = hlib->local.play_cur + jpeg_size;
...
static void JMV_FillAudioFIFO(audio_lib_handle_t * hlib)
{
    ...
    if ((hlib->local.play_cur + offset_bytes + audio_pkt_size) >= hlib->fp->fptr) return;
    
    /* Save network pointers (Rewind) */
    orig_rptr = hlib->local.buffer.rptr;
    orig_play_cur = hlib->local.play_cur;
    
    /* Skip to desired audio block */
    hlib->local.buffer.rptr = (orig_rptr + offset_bytes) % hlib->local.buffer.size;
    hlib->local.play_cur = orig_play_cur + offset_bytes;
    ...
    hlib->local.buffer.rptr = orig_rptr;
    hlib->local.play_cur = orig_play_cur;
