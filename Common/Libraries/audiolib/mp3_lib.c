...
#define MP3_LIB_READBUFF_SIZE  (256 * MP3_LIB_READBUF_SIZE)

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static audio_lib_buffer_t mp3_buffer[MP3_LIB_AUDIOBUF_NUM];
static audio_lib_read_state_t read_state;
static mp3_lib_codec_t mp3_codec;
static mp3_lib_state_t mp3_lib_state;

/* Private function prototypes -----------------------------------------------*/
void    mp3_lib_process(audio_lib_handle_t * hlib);
void    mp3_lib_seek(audio_lib_handle_t * hlib);
...
void mp3_lib_process(audio_lib_handle_t * hlib)
{
    ...
    switch (mp3_lib_state)
    {
        case MP3_LIB_INIT:
            /* Allocate net buffer */
            hlib->local.buffer.size = MP3_LIB_READBUFF_SIZE;
            ...
            mp3_lib_state = MP3_LIB_FILL_READBUFF;
            break;
            
        case MP3_LIB_FILL_READBUFF:
            switch (read_state)
            {
        ...
        case MP3_LIB_PROC_SAMPLES:
            switch(hlib->playback_state)
            {
                case AUDIO_LIB_STATE_PLAY:
                    hlib->time.elapsed_time = (hlib->local.play_cur * 8) / ((codec->info.bitrate / 1000) * 1024);
...
void mp3_lib_seek(audio_lib_handle_t * hlib)
{
    ...
    hlib->time.elapsed_time = ((uint64_t)hlib->fp->fptr * 8) / ((codec->info.bitrate / 1000) * 1024);    
}

void mp3_lib_free(audio_lib_handle_t * hlib)
{
    mp3_lib_codec_t * codec;
...
static void MP3_DecodeFrame(audio_lib_handle_t * hlib)
{
    static mp3_lib_decode_state_t decode_state;
