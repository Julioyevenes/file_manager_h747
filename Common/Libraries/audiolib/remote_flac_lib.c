...
{
    FLAC_LIB_INIT = 0,
    FLAC_LIB_FILL_NETBUFF,
    FLAC_LIB_FIRST_SAMPLE,
    FLAC_LIB_PROC_SAMPLES
} remote_flac_lib_state_t;
...
typedef struct
{
	...
	remote_flac_lib_frame_t		frame;
	remote_flac_lib_metadata_t		metadata;
} remote_flac_lib_codec_t;

/* Private constants ---------------------------------------------------------*/
#define FLAC_LIB_AUDIOBUF_SIZE	        (0x9000)	/* 36864 byte */
#define FLAC_LIB_AUDIOBUF_NUM	        2

#define REMOTE_FLAC_LIB_NETBUFF_SIZE    (2 * 1024 * 1024)

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static audio_lib_buffer_t flac_buffer[FLAC_LIB_AUDIOBUF_NUM];
static audio_lib_read_state_t remote_read_state;
static remote_flac_lib_codec_t flac_codec;
static remote_flac_lib_state_t flac_lib_state;

/* Private function prototypes -----------------------------------------------*/
void 									remote_flac_lib_process(audio_lib_handle_t * hlib);
void                                    remote_flac_lib_seek(audio_lib_handle_t * hlib);
void 									remote_flac_lib_free(audio_lib_handle_t * hlib);
void 									remote_flac_lib_transfer_complete_cb(audio_lib_handle_t * hlib);
void 									remote_flac_lib_transfer_half_cb(audio_lib_handle_t * hlib);
...
const audio_lib_t remote_flac_lib = 
{
	NULL,
	remote_flac_lib_process,
	remote_flac_lib_seek,
	remote_flac_lib_free,
	remote_flac_lib_transfer_complete_cb,
	remote_flac_lib_transfer_half_cb
};
...
void remote_flac_lib_process(audio_lib_handle_t * hlib)
{
	...
	remote_flac_lib_codec_t * codec = hlib->prv_data;
    ...
    switch (flac_lib_state)
    {
        case FLAC_LIB_INIT:
            /* Allocate net buffer */
            hlib->remote.buffer.size = REMOTE_FLAC_LIB_NETBUFF_SIZE;
        ...
        case FLAC_LIB_FIRST_SAMPLE:
            ...
            if (FLAC__stream_decoder_process_until_end_of_metadata(flac_codec.h) == true)
            {
                ...
                flac_codec.bitrate = (uint32_t) (((float) hlib->remote.fsize / flac_codec.metadata.total_samples) * flac_codec.metadata.sample_rate * 8);
                ...
                hlib->time.total_time = (hlib->remote.fsize * 8) / flac_codec.bitrate;
            }
            else
            {
                hlib->err = AUDIO_LIB_READ_ERROR;
        ...
        case FLAC_LIB_PROC_SAMPLES:
            switch(hlib->playback_state)
            {
                case AUDIO_LIB_STATE_PLAY:
                    hlib->time.elapsed_time = (hlib->remote.play_cur * 8) / codec->bitrate;
...
static FLAC__StreamDecoderTellStatus flac_lib_tell_cb(const FLAC__StreamDecoder* decoder, FLAC__uint64* absolute_byte_offset, void* client_data)
{
    ...
    if (hlib->remote.play_cur < 0) {
        // seek failed
        return FLAC__STREAM_DECODER_TELL_STATUS_ERROR;
    }
    else {
        // update offset
        *absolute_byte_offset = (FLAC__uint64) hlib->remote.play_cur;
...
static FLAC__StreamDecoderLengthStatus flac_lib_length_cb(const FLAC__StreamDecoder* decoder, FLAC__uint64* stream_length, void* client_data)
{
    ...
    if (hlib->remote.fsize == 0) {
        // failed
        return FLAC__STREAM_DECODER_LENGTH_STATUS_ERROR;
    }
    else {
        // pass on length
        *stream_length = (FLAC__uint64) hlib->remote.fsize;
...
static FLAC__bool flac_lib_eof_cb(const FLAC__StreamDecoder* decoder, void* client_data)
{
	...
	return (hlib->remote.play_cur == hlib->remote.fsize);
}
