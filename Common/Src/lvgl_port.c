...
#define LVGL_MOUSE_SENS_NUM     15
#define LVGL_MOUSE_SENS_DEN     10
#define LVGL_MOUSE_ACCEL_THRES  10

#define LV_SYMBOL_POINTER "\xEF\x89\x85"

/* Private macro ------------------------------------------------------------*/
/* Private variables --------------------------------------------------------*/
extern lv_font_t                    arrow_pointer_20;
...
static uint8_t  lvgl_jpeg_decoder_read_cb(void *arg, void* buff, uint32_t btr, uint32_t* br);
...
void lvgl_disp_init(void)
{
    ...
    /** 
      ...
      */
	  BSP_LCD_InitEx(LCD_RGB565_800_480);
...
void lvgl_indev_init(void)
{
    ...
    if(BSP_TS_Init(BSP_LCD_GetXSize(), BSP_LCD_GetYSize()) == TS_OK)
    {
        ...
        ts_indev = lv_indev_drv_register(&indev_drv);
    }
    ...
    ptr_indev = lv_indev_drv_register(&indev_drv);

    /* Set cursor */
    lv_style_init(&style_cursor);
    lv_style_set_text_font(&style_cursor, LV_STATE_DEFAULT, &arrow_pointer_20);
    lv_style_set_image_recolor(&style_cursor, LV_STATE_DEFAULT, LV_COLOR_SILVER);
    
    lv_obj_t * mouse_cursor = lv_img_create(lv_disp_get_scr_act(NULL), NULL);
    lv_obj_add_style(mouse_cursor, LV_IMG_PART_MAIN, &style_cursor);
    lv_img_set_src(mouse_cursor, LV_SYMBOL_POINTER);
...
static bool lvgl_mouse_read_cb(lv_indev_drv_t *indev, lv_indev_data_t *data)
{
    ...
    /*Save the pressed coordinates and the state*/
    if(m_pinfo != NULL) {
        /* Extract the deltas while preserving the sign */
        int16_t delta_x = (int8_t) m_pinfo->x;
        int16_t delta_y = (int8_t) m_pinfo->y;

        /* Only process if there was physical movement */
        if (delta_x != 0 || delta_y != 0) {
            /* Base Sensitivity */
            delta_x = (delta_x * LVGL_MOUSE_SENS_NUM) / LVGL_MOUSE_SENS_DEN;
            delta_y = (delta_y * LVGL_MOUSE_SENS_NUM) / LVGL_MOUSE_SENS_DEN;

            /* Dynamic Acceleration */
            int16_t abs_x = (delta_x < 0) ? -delta_x : delta_x;
            int16_t abs_y = (delta_y < 0) ? -delta_y : delta_y;

            if (abs_x > LVGL_MOUSE_ACCEL_THRES || abs_y > LVGL_MOUSE_ACCEL_THRES) {
                delta_x *= 2;
                delta_y *= 2;
            }

            /* Add the processed deltas to the screen coordinates */
            last_x += delta_x;
            last_y += delta_y;
        }
...
static lv_res_t lvgl_img_decoder_info_cb(struct _lv_img_decoder * decoder, const void * src, lv_img_header_t * header)
{
    ...
    if(src_type == LV_IMG_SRC_FILE)
    {
           ...
           !strcmp(&fn[strlen(fn) - 3], "jpg"))
        {
            ...
            {
                /* Init JPEG codec */
                lvgl_img_hjpegcodec.mode = JPEG_CODEC_MODE_IT;
                jpeg_decoder_init(&lvgl_img_hjpegcodec,
                                  &lvgl_img_hjpeg,
                                  &lvgl_img_fh,
                                  lvgl_jpeg_decoder_read_cb,
                                  0,
                                  lvgl_img_addr);
                ...
                if(lvgl_img_jpeginfo.ChromaSubsampling == JPEG_444_SUBSAMPLING)
                    header->css = 0x0;
                else if(lvgl_img_jpeginfo.ChromaSubsampling == JPEG_422_SUBSAMPLING)
                    header->css = 0x1;
                else /* if(lvgl_img_jpeginfo.ChromaSubsampling == JPEG_420_SUBSAMPLING) */
                    header->css = 0x2;
    ...
    {
        memcpy(header, &((lv_img_dsc_t *)src)->header, sizeof(lv_img_header_t));
        return LV_RES_OK;
    }
...
uint8_t lvgl_jpeg_decoder_read_cb(void *arg, void* buff, uint32_t btr, uint32_t* br)
{
    FIL * fp = (FIL *) arg;
    return f_read(fp, buff, btr, (UINT *)br);
}
