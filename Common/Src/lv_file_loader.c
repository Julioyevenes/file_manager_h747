...
#define ADDR_FLASH_SECTOR_0     ((uint32_t)0x08100000) /* Base @ of Sector 0, 128 Kbytes */
#define ADDR_FLASH_SECTOR_1     ((uint32_t)0x08120000) /* Base @ of Sector 1, 128 Kbytes */
#define ADDR_FLASH_SECTOR_2     ((uint32_t)0x08140000) /* Base @ of Sector 2, 128 Kbytes */
#define ADDR_FLASH_SECTOR_3     ((uint32_t)0x08160000) /* Base @ of Sector 3, 128 Kbytes */
#define ADDR_FLASH_SECTOR_4     ((uint32_t)0x08180000) /* Base @ of Sector 4, 128 Kbytes */
#define ADDR_FLASH_SECTOR_5     ((uint32_t)0x081A0000) /* Base @ of Sector 5, 128 Kbytes */
#define ADDR_FLASH_SECTOR_6     ((uint32_t)0x081C0000) /* Base @ of Sector 6, 128 Kbytes */
#define ADDR_FLASH_SECTOR_7     ((uint32_t)0x081E0000) /* Base @ of Sector 7, 128 Kbytes */
...
{
	uint32_t i, j;
	uint64_t flashword[4] = {0};

	for(i = 0; i < (size / 32); i++)
	{
		/* Compute the flash word */
		for(j = 0; j < 4; j++)
		{
			flashword[j] = ((uint64_t) buf_addr[2 * j + 1 + 8 * i] << 32) | (buf_addr[2 * j + 8 * i] & 0x00000000FFFFFFFF);
		}

		if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_FLASHWORD, (uint32_t) addr[0], (uint32_t) flashword) == HAL_OK)
		{
			for(j = 0; j < 4; j++)
			{
				/* Check the written value */
				if(*(uint64_t*)addr[0] != flashword[j])
				{
...
void lv_fm_loader_copy_task(lv_task_t * task)
{
	...
	{
		h->err = lv_fm_loader_mem_write(&(h->cpy_addr), (uint32_t *) &(h->cpy_buf), bytesread);
...
void lv_fm_loader_bar_task(lv_task_t * task)
{
	...
	lv_snprintf(buf, sizeof(buf), "Loading %d  /  %d  KB", value, total);
